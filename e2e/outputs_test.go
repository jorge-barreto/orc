...
// TestOutputs_MissingOutputTriggersRepromptThenFails verifies that when
// declared outputs are missing, the agent is re-prompted ONCE; if the
// output is still missing, the run fails and dispatch-counts shows 2
// attempts.
func TestOutputs_MissingOutputTriggersRepromptThenFails(t *testing.T) {
	if os.Getenv("CLAUDE_CODE_OAUTH_TOKEN") == "" {
		t.Skip("CLAUDE_CODE_OAUTH_TOKEN not set; skipping agent-phase test")
	}

	cfg := `name: outputs-fail
phases:
  - name: agent
    type: agent
    prompt: .orc/prompts/outputs-noop.md
    model: haiku
    outputs: [impossible_file.txt]
`
	w := NewWorkspace(t, cfg)
	w.Ticket = "OUTPUTS-FAIL-001"
	w.ArtifactsDir = strings.Replace(w.ArtifactsDir, "TEST-1", "OUTPUTS-FAIL-001", 1)

	promptBytes, _ := os.ReadFile("prompts/outputs-noop.md")
	w.WritePrompt("prompts/outputs-noop.md", string(promptBytes))

	res := w.RunOrc("run", w.Ticket, "--auto")
	if res.ExitCode == 0 {
		t.Errorf("exit = 0, want non-zero (output never produced)\nstdout: %s\nstderr: %s",
			res.Stdout, res.Stderr)
	}

	// State should show failure category output_missing.
	state := w.ReadState()
	if got := state["failure_category"]; got != "output_missing" {
		t.Errorf("failure_category = %v, want output_missing", got)
	}

	// dispatch-counts.json should show 2 attempts (original + 1 re-prompt).
	auditPath := filepath.Join(w.Dir, ".orc", "audit", w.Ticket, "dispatch-counts.json")
	data, err := os.ReadFile(auditPath)
	if err != nil {
		t.Fatalf("read dispatch-counts: %v", err)
	...
	if c := counts["0"]; c != 2 {
		t.Errorf("dispatch-counts[0] = %d, want 2 (initial + 1 re-prompt)", c)
	}
}
