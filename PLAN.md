...
**Why now:** The current `/ticket` workflow in the Acme project relies on an LLM agent to self-orchestrate 10 phases from a 300-line monolithic prompt. The agent is responsible for knowing what phase it's in, doing the right work, and calling checkpoints. If it skips steps, misinterprets boundaries, or loses context, guardrails don't fire until after the damage. Deterministic phases burn LLM tokens unnecessarily. Inspired by Stripe's "blueprint" pattern (Minions Part 2), we invert control.

**Two-phase build:** First we build orc as a standalone open-source tool at `~/work/orc/`. Then we create an orc workflow definition for the Acme project. orc is repo-agnostic — any project provides a `.orc/config.yaml` and phase prompts, and the engine runs them.
...
## Part B: Acme Project Workflow (in `acme-service`)

After orc is built and working, create the workflow definition for Acme.

### New Files

**`.orc/config.yaml`** (~50 lines)

Acme's workflow — uses the full config syntax including `on-fail` loops, `outputs` validation, per-phase `model` selection, and `timeout`:
...
**`.orc/phases/self-review.md`** (~80 lines) — Embedded review checklist (correctness, security, naming, tests, Acme-specific patterns). Identifies blocking issues and writes `review-synthesis.md`. Does NOT fix code — if blocking issues are found, exits non-zero so `on-fail` loops back to implement with the findings as feedback context.
...
### Part B — Acme Blueprint (`acme-service`)
9. `.orc/config.yaml` — Acme phase graph
